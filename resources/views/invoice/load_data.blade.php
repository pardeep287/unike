...
<tr>
    ...
    <th width="18%">
        ...
        <a href="javascript:void(0)" class="sort" data-sort-action="{!! $sortOrderNumberAction !!}" data-sort-entity="invoice_number">
            {!! lang('invoice.inv_number') !!}
    ...
    <th>
        ...
        <a href="javascript:void(0)" class="sort" data-sort-action="{!! $sortInvoiceDateAction !!}" data-sort-entity="invoice_date">
            {!! lang('invoice.inv_date') !!}
    ...
    <th >{!! lang('invoice.gross_amount') !!}</th>
    {{--<th class="text-center" width="10%">{!! lang('common.email_send') !!}</th>--}}
    @if(hasMenuRoute('invoice.edit') || isAdmin() || hasMenuRoute('invoice.order-adjustment'))
...
<tr id="order_{{ $detail->id }}">
    ...
    <td>
    @if(hasMenuRoute('invoice.edit') || isAdmin())
     <a title="{!! lang('common.view') !!}" href="{!! route('invoice.edit', [$detail->id]) !!}">
      {!! $detail->order_number !!}
     </a>
    @else
     {!! $detail->order_number !!}
    ...
    <td class="text-center">
      @if(hasMenuRoute('invoice.edit') || isAdmin())
        <a data-title="{!! lang('common.view_order_item_detail') !!}" class="btn btn-xs dEdit btn-success hidden" href="javascript:void(0)" data-setting="modal-lg" data-route="{{ route('invoice.item-detail', [$detail->id]) }}">
            <i class="fa fa-eye"></i>
        </a>

        <a title="{!! lang('common.print') !!}" class="btn btn-xs btn-default" href="{{ route('invoice.invoice-print', [$detail->id]) }}">
            <i class="fa fa-print"></i>
        </a>

        <a title="{!! lang('common.edit') !!}" class="btn btn-xs btn-primary" href="{{ route('invoice.edit', [$detail->id]) }}">
            <i class="fa fa-edit"></i>
        </a>

        <a title="{!! lang('common.delete') !!}" class="btn btn-xs btn-danger __drop hidden" data-route="{!! route('invoice.drop', [$detail->id]) !!}" data-message="{!! lang('messages.sure_delete', string_manip(lang('invoice.invoice'))) !!}" href="javascript:void(0)">
